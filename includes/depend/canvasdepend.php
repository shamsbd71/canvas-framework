 ...
 */
class JFormFieldCANVASDepend extends JFormField
{
	/**
	 * Element name
	 *
	...
	function loadAsset(){
		if (!defined ('_CANVAS_DEPEND_ASSET_')) {
			...
			if(JFactory::getApplication()->isSite() || !defined('CANVAS_TEMPLATE')){
    ...
    protected function profile()
    {
        ...
        if (file_exists($xmlparams)) {
            /* For General Form */
            $canvasform = JForm::getInstance('jform', $xmlparams, array('control' => 'canvasform'));
		...
		if($level==1){
			$html = '
				<h4 rel="'.$level.'" class="block-head block-head-'.$class_name.' open '.$class.' " '.$group.' id="'.$regionID.'">
					...
					<a class="toggle-btn open" title="'.JText::_('Expand all').'" onclick="CANVASDepend.showseg(\''.$regionID.'\', \'level'.$level.'\'); return false;">'.JText::_('Expand all').'</a>
					<a class="toggle-btn close" title="'.JText::_('Collapse all').'" onclick="CANVASDepend.showseg(\''.$regionID.'\', \'level'.$level.'\'); return false;">'.JText::_('Collapse all').'</a>
		    </h4>';
		} else {
			$html = '
				<h4 rel="'.$level.'" class="block-head block-head-'.$class_name.' open '.$class.' " '.$group.' id="'.$regionID.'">
					<span class="block-setting" >'.$_title.$_url.'</span> 
					<span class="icon-help editlinktip hasTip" title="'.htmlentities($_description).'">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
					<a class="toggle-btn" title="'.JText::_('Click here to expand or collapse').'" onclick="CANVASDepend.segment(\''.$regionID.'\', \'level'.$level.'\'); return false;">open</a>
		    </h4>';
		} 
	...
	function group(){
		...
		<script type="text/javascript">
			...
			?>
				CANVASDepend.add('<?php echo $option['for']; ?>', {
					vals: '<?php echo $vals ?>',
					elms: '<?php echo $elms?>',
	...
	function ajax(){
		...
		foreach ($this->element->children() as $option):
			...
			$fcalls[] = 'CANVASDepend.addajax(\'' . $this->getName($option['for']) . '\', ' . json_encode($fparams) . ');';
		endforeach;
		?>
		<script type="text/javascript">
			//<![CDATA[
	...
	function legend(){
		return '<legend class="canvas-admin-form-legend">' . JText::_($this->element['label']) . '<small class="canvas-admin-form-legend-desc">' . JText::_($this->element['description']) . '</small> </legend>';
	}
} 