...
class JFormFieldCANVASMegaMenu extends JFormFieldHidden
{
	...
	protected function getMegaMenuMarkup()
	{
		...
		if(!defined('CANVAS_TEMPLATE')){
			$this->loadCANVASDepend();
		}

		$canvaspath = CANVAS_ADMIN_PATH;
		
		if(!defined('__CANVAS_MEGAMENU_ASSET__')){
			define('__CANVAS_MEGAMENU_ASSET__', 1);

			$jdoc = JFactory::getDocument();

			if(is_file(CANVAS_PATH . '/css/megamenu.css')){
				$jdoc->addStylesheet(CANVAS_URL . '/css/megamenu.css');
			}

			if(is_file(CANVAS_ADMIN_PATH . '/admin/megamenu/css/megamenu.css')){
				$jdoc->addStylesheet(CANVAS_ADMIN_URL . '/admin/megamenu/css/megamenu.css');
			}
			...
			if(is_file(CANVAS_ADMIN_PATH . '/admin/megamenu/js/megamenu.js')){
				$jdoc->addScript(CANVAS_ADMIN_URL . '/admin/megamenu/js/megamenu.js');
			}
		}
